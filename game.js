...
<head>
    ...
    <title>EEVEE TEACHES TYPING</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <script>
        ...
        // Function to check for three identical digits
        function hasThreeIdenticalDigits(number) {
            const str = number.toString();
            const digitCount = {};
            for (let char of str) {
                digitCount[char] = (digitCount[char] || 0) + 1;
                if (digitCount[char] >= 3) return true;
            }
            return false;
        }
        ...
        class MainGameScene extends Phaser.Scene {
            ...
            create(data) {
                ...
                this.timeText = this.add.text(10, floorY + 20, 'Time: 0s', { font: '24px "Press Start 2P"', fill: '#000000' });
                this.wpmText = this.add.text(10, floorY + 50, 'WPM: 0', { font: '24px "Press Start 2P"', fill: '#000000' });
                this.missedText = this.add.text(10, floorY + 80, 'Missed: 0', { font: '24px "Press Start 2P"', fill: '#000000' });
                this.scoreText = this.add.text(this.cameras.main.width / 2, floorY + 120, '0000000', { font: '48px "Press Start 2P"', fill: '#000000' }).setOrigin(0.5);
                this.keyText = this.add.text(this.cameras.main.width - 100, floorY + 20, '', { font: '24px "Press Start 2P"', fill: '#000000' }).setOrigin(1, 0);
                const meterY = 20;
                const meterHeight = 24;
                this.bonusMeterBg = this.add.rectangle(this.cameras.main.width / 2 - 102, meterY, 204, meterHeight, 0x000000);
                this.bonusMeterRed = this.add.rectangle(this.cameras.main.width / 2 - 100, meterY + 2, 200, meterHeight - 4, 0xff0000);
                this.bonusMeterFill = this.add.rectangle(this.cameras.main.width / 2 - 100, meterY + 2, 0, meterHeight - 4, 0xffff00);
                this.bonusMeterText = this.add.text(this.cameras.main.width / 2, meterY + meterHeight / 2, 'BONUS', { font: '24px "Press Start 2P"', fill: '#ff0000' }).setOrigin(0.5);
                ...
                this.nextGoalsText = this.add.text(10, floorY + 110, '', { font: '40px "Press Start 2P"', fill: '#000000' });
            ...
            update(time) {
                ...
                if (gameState.totalTime > 0 && hasThreeIdenticalDigits(gameState.totalTime) && !this.secretBox) this.spawnSecretBox();
                ...
                if (this.secretBox) {
                    this.secretBox.time += this.time.delta / 1000;
                    this.secretBox.y = this.secretBox.originalY + 20 * Math.sin(this.secretBox.time * Math.PI * 2);
                    this.secretBox.x -= 5;
            ...
            spawnSecretBox() {
                ...
                this.secretBox.originalY = y;
                this.secretBox.time = 0;
            }
            ...
            handleKeyPress(event) {
                ...
                if (goal && event.key === goal.key) {
                    gameState.paused = false;
                    this.character.setTexture('eevee-jumping');
                    this.tweens.add({
                        targets: this.character,
                        x: goal.x,
                        y: goal.y,
                    ...
                    if (!this.secretBox || (event.key !== 'e' && event.key !== 'v')) {
        ...
        class BossFightScene extends Phaser.Scene {
            ...
            update() {
                    ...
                    if (this.currentWord.x <= this.character.x) {
                        ...
                        if (this.character) {
                            ...
                            this.time.delayedCall(3000, () => {
                                if (this.character && this.scene.isActive()) {
                                    this.character.setTexture('eevee-standing').play('eevee-standing');
                                }
                            });
                        }
                ...
                if (this.boss && this.boss.health <= 0) this.endBossFight();
            }
